...
extension Character {
    func isVowel() -> Bool {
        ...
        return vowels.contains(Character(self.lowercased()))
    }
}
...
let bigRect = Rectangle(length: 10.0, width: 6.0)
let smallRect = bigRect.halved()

print("Original: \(bigRect.length) x \(bigRect.width)")
print("Halved: \(smallRect.length) x \(smallRect.width)")
*/
...
print("myRectangle: length = \(myRectangle.length), width = \(myRectangle.width)")
print("mySmallerRectangle: length = \(mySmallerRectangle.length), width = \(mySmallerRectangle.width)")
